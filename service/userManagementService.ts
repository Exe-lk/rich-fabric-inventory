...
import { getAuth, createUserWithEmailAndPassword, deleteUser as deleteAuthUser , signInWithEmailAndPassword } from 'firebase/auth';

export const createUser = async (data:any) => {
  const userCredential = await createUserWithEmailAndPassword(auth, data.email, data.nic);
  const user = userCredential.user;
const docRef = await addDoc(collection(firestore, 'UserManagement'), data);
  return docRef.id;
};
